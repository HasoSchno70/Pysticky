...
_on_stitch_unmarked_completed() in ui/handlers/undo_handlers.py. Diese
Handler und die zugrundeliegenden Layer.mark_completed()/unmark_completed()
pruften bisher NICHT, ob die gefundene Ebene gesperrt (locked) ist -- anders
als set_stitch()/clear()/remove_stitch(), die eine gesperrte Ebene konsequent
vor jeder Grid-Mutation schuetzen (siehe test_eraser_noop_undo_guard.py fuer
dieselbe Fehlerklasse beim Radiergummi-Werkzeug).

Dadurch konnte man auf einer gesperrten Ebene trotzdem den
Fortschritt-Haken setzen/entfernen -- UND es landete dabei ein komplett
wirkungsloser Eintrag auf dem Undo-Stack (Strg+Z "verpuffte" ohne sichtbare
Aenderung).

Fix:
- Layer.mark_completed()/unmark_completed() lehnen jetzt wie set_stitch()
  jede Mutation auf einer gesperrten Ebene ab (core/layer.py).
- _on_stitch_marked_completed()/_on_stitch_unmarked_completed() ueberspringen
  jetzt das Erzeugen des Commands komplett, wenn die gefundene Ebene
  gesperrt ist (ui/handlers/undo_handlers.py) -- kein wirkungsloser
  Undo-Eintrag mehr.
"""
...
def test_layer_mark_completed_respects_lock():
    """Layer.mark_completed() direkt: gesperrte Ebene bleibt unveraendert."""
...
def test_layer_unmark_completed_respects_lock():
    """Layer.unmark_completed() direkt: gesperrte Ebene bleibt unveraendert."""
    from pysticky.core import Layer
...
def test_marking_completed_on_locked_layer_creates_no_undo_entry(qtbot):
...
def test_unmarking_completed_on_locked_layer_creates_no_undo_entry(qtbot):
    from pysticky.core import Pattern
...
def test_marking_completed_on_unlocked_layer_still_works(qtbot):
    """Der neue Guard darf den regulaeren Fall nicht mit-blockieren."""
    from pysticky.core import Pattern
