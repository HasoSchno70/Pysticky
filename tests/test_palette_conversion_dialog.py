...
def test_apply_warns_on_poor_match_matching_the_red_table_threshold(qtbot, monkeypatch):
    """Regressionstest (Runde 20): die "Schlechte Zuordnungen"-Warnung
    schwellte vorher erst bei distance > 60, waehrend die Tabelle Zeilen
    schon ab distance > 25 rot ("poor") einfaerbt (Zeile ~403 im Dialog).
    Ein Nutzer konnte dadurch sichtbar rote Zeilen anklicken, ohne je eine
    Warnung zu sehen. Beide Schwellen muessen jetzt uebereinstimmen (25)."""
    pattern = _make_pattern()
    dialog = PaletteConversionDialog(pattern)
    qtbot.addWidget(dialog)
    ...
    target_b = Thread.from_hex("Ziel B", "#444444", manufacturer="Anchor", catalog_number="10")
    dialog._mapping = [
        {"entry": pattern.color_entries[0], "target_thread": target_a, "distance": 30.0},
        {"entry": pattern.color_entries[1], "target_thread": target_b, "distance": 1.0},
    ]
...
def test_apply_does_not_warn_below_poor_match_threshold(qtbot, monkeypatch):
    pattern = _make_pattern()
    dialog = PaletteConversionDialog(pattern)
    qtbot.addWidget(dialog)
    ...
    target_b = Thread.from_hex("Ziel B", "#444444", manufacturer="Anchor", catalog_number="10")
    dialog._mapping = [
        {"entry": pattern.color_entries[0], "target_thread": target_a, "distance": 20.0},
        {"entry": pattern.color_entries[1], "target_thread": target_b, "distance": 1.0},
    ]
